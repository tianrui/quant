...
def p2_opt():
    ...
    with g.as_default(): 
        ...
        #nsharpe = (-1) * tf.reduce_sum(tf.truediv( tf.truediv(cumsum(RP2), powers), tf.pow(cumsum(tf.pow(RP2 - tf.truediv(cumsum(RP2), powers), 2)), 0.5)))
    ...
    with tf.Session(graph=g) as session:
        ...
        for step in range(num_steps):
            ...
            rvals = session.run([optimizer, nsharpe, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, RP2], feed_dict=feed_dict)
            loss = rvals[1]
            if loss < lowest_loss and min(rvals[-1]) > -1.:
                save_path = np.savetxt('./checkpoints/modelp2.ckpt', rvals[1:14], fmt='%10.5f', delimiter=',')
...
def p3_opt():
    ...
    with g.as_default(): 
        ...
        plW2 = tf.placeholder(tf.float32, shape=[None, RCC.shape[1]], name='plW2')
        plIND = tf.placeholder(tf.float32, shape=[None, RCC.shape[1]], name='plW2')
        sharpe = tf.placeholder(tf.float32, shape=[None])
        ...
        plW2 = plW2 + tf.mul(a10, plB10)
        plW2 = plW2 + tf.mul(a11, plB11)
        F3 = tf.to_float(tf.greater_equal(plW2 * plIND, tf.zeros_like(plW2)))
        
        W2_ROC = tf.reduce_sum(plW2 * plROC, reduction_indices=1)
        norm_W2 = tf.reduce_sum(tf.abs(plW2), reduction_indices=1)
        powers = (tf.range(1, batch_size+1))

        RP2 = tf.truediv(tf.reduce_sum(F3 * plW2 * plROC, reduction_indices=1), tf.reduce_sum(tf.abs(F3 * plW2), reduction_indices=1))
    ...
    with tf.Session(graph=g) as session:
        ...
        for step in range(num_steps):
            offset = np.random.randint(200, 500) #(step * batch_size) % (RCC.shape[0] - 2 - batch_size)
            batchROC = ROC[2 + offset:(2 + offset + batch_size)]
            batchIND = IND[2 + offset:(2 + offset + batch_size)]
            batchB0 = B[0][offset:(offset + batch_size)]
            batchB1 = B[1][offset:(offset + batch_size)]
            batchB2 = B[2][offset:(offset + batch_size)]
            batchB3 = B[3][offset:(offset + batch_size)]
            ...
            rvals = session.run([optimizer, nsharpe, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, F3], feed_dict=feed_dict)
            loss = rvals[1]
            if loss < lowest_loss:
                save_path = np.savetxt('./checkpoints/modelp3.ckpt', rvals[1:14], fmt='%10.5f')
                print "Model saved in %s" % save_path
                print('Sharpe ratio at step %d: %f'%(step, -loss))
                lowest_loss = loss
            a_vals = rvals[2:14]
